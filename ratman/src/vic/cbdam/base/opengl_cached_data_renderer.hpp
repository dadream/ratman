...
namespace cbdam {
   ...
   *
   */
  class opengl_cached_data_renderer {
  public:
  ...
  public:
    ...
    void set_projection_parameters(const coordinate_transform* geo_xform);

    void set_elevation_range(float min_h, float max_h);

    void set_vbo_parameters(uint32_t height_patch_dim, uint32_t color_patch_dim);
    
    void init_opengl();
    
    void frame_initialize(const point4_t& surface_color);

    void frame_finalize();

    void bind_texture(const diamond_patch_accessor_t* x);
   
    void render(const diamond_patch_id_t& node_idx, const diamond_patch_accessor_t* data_ptr);

    bool has(const diamond_patch_id_t& node_idx) const;

    void set_texture_cache_capacity(uint32_t capacity_bytes);

    void set_elevation_map_enabled(bool x);
    bool elevation_map_enabled() const;

    rendering_mode_t gl_rendering_mode() const;

    void set_use_color(bool x);
    bool use_color() const;
    void set_use_normal(bool x);
    bool use_normal() const;
    uint32_t current_vbo_cache_count() const;
